...
You can create a request using the ``PvGuzzleHttp\Psr7\Request`` class:
...
You can create a response using the ``PvGuzzleHttp\Psr7\Response`` class:
...
.. code-block:: php
    ...
    $response = PvGuzzleHttp\get('http://httpbin.org/get');
...
function from the ``PvGuzzleHttp\Psr7`` namespace --
``PvGuzzleHttp\Psr7\stream_for``. This function accepts strings, resources,
...
.. code-block:: php
    ...
    use PvGuzzleHttp\Stream\Stream;
...
.. code-block:: php
    ...
    $request = new \PvGuzzleHttp\Psr7\Request('MOVE', 'http://httpbin.org/move');
...
The best way to create a stream is using the ``PvGuzzleHttp\Psr7\stream_for``
